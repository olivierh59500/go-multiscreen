...
The Phenomena and TCB scenes use the same `DNAFrames`, `SliceProgram`, `RecurrentRowWave`, `ScalarStages`, reusable gradient/silhouette materials, `Planes` and `PlaneRenderer` implementations as their standalone DCK demos. Their artwork, authored layer order and the multiscreen camera remain local.
